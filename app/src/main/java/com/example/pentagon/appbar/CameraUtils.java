...
public class CameraUtils {
    ...
    public static File getOutputMediaFile(int type) {
        ...
        File mediaStorageDir = new File(
                Environment
                        .getExternalStoragePublicDirectory("MYAPPP"),
                Main2Activity.GALLERY_DIRECTORY_NAME);

        // Create the storage directory if it does not exist
        if (!mediaStorageDir.exists()) {
            if (!mediaStorageDir.mkdirs()) {
                Log.e(Main2Activity.GALLERY_DIRECTORY_NAME, "Oops! Failed create "
                        + Main2Activity.GALLERY_DIRECTORY_NAME + " directory");
                return null;
            }
        }
        ...
        else if(type == Main2Activity.MEDIA_TYPE_AUDIO){

            mediaFile = new File(mediaStorageDir.getPath() + File.separator
                    + "AUD_" + timeStamp + "." + Main2Activity.AUDIO_EXTENSION);
        }
